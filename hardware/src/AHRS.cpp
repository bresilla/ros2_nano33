...
void AHRS::formatAnglesForConfigurator() {
...
BoardType AHRS::getBoardType() {
  return _boardType;
}

const char* AHRS::getBoardTypeString() {
  uint8_t index = (uint8_t)_boardType;

  return _boardTypeStr[index];
}

void AHRS::setFusionAlgorithm(SensorFusion algo) {
  _fusion = algo;
}

void AHRS::setAlpha(float a) {
  _alpha = a;
}

void AHRS::setBeta(float b) {
  _beta = b;
}

void AHRS::setGyroMeasError(float gme) {
  _gyroMeasError = gme;
}

void AHRS::setKp(float p) {
  _Kp = p;
}

void AHRS::setKi(float i) {
  _Ki = i;
}

void AHRS::setDeclination(float dec) {
  _declination = dec;
}

void AHRS::setData(SensorData d, bool axisAlign) {
...
void AHRS::setDOF(DOF d) {
  _dof = d;
}

void AHRS::setBoardType(BoardType b) {
  _boardType = b;
}

void AHRS::setImuType(ImuType i) {
  _imuType = i;
}
...
void AHRS::kalmanUpdate(float deltaT) {
...
void AHRS::updateEulerAngles(float deltaT) {
...
void AHRS::complementaryUpdate(SensorData d, float deltaT) {
...
void AHRS::madgwickUpdate(SensorData d, float deltaT) {
  float norm;
...
void AHRS::mahoneyUpdate(SensorData d, float deltaT) {
  float norm;
  float hx, hy, bx, bz;
